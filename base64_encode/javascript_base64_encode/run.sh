...
export OS=$(uname -s)
TARGET_SNAKE=$(basename $(pwd) | sed -e -e 's/^[^_]*_//')
...
if [ -f ${TARGET_SNAKE}.${EXT} ]; then
  ...
  if echo ${ANSWER} | grep -iq "^y" ; then
    ...
    if [ "$(uname -s)" = "FreeBSD" ]; then
      sed -e -e -e -e -e -ir "s/${EXAMPLE_SNAKE}/${TARGET_SNAKE}/g" ${TARGET_SNAKE}.${EXT}
    else
      sed -e -e -e -e -e -i "s/${EXAMPLE_SNAKE}/${TARGET_SNAKE}/g" ${TARGET_SNAKE}.${EXT}
    fi
  else
    echo
  fi
